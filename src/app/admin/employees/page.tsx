...
import { listEmployeesAdmin } from "@/server/crud";
    ...
    <AdminChrome userName={session.name}>
      <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="font-serif text-2xl font-semibold">Сотрудники</h1>
            <p className="mt-1 text-sm text-muted">Педагогический состав и руководство</p>
          </div>
          <Link
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-line last:border-b-0">
                  ...
                  <td className="px-4 py-3 text-graphite">{item.role}</td>
